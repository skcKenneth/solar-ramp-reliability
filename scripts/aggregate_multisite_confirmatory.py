...
from run_advanced_study import interval_metrics, point_metrics  # noqa: E402
...
def summarize_chunk(path: Path, alpha: float, min_group_n: int, capacity: float, station: str) -> tuple[list[dict], list[dict], list[dict]]:
    pred = pd.read_pickle(path, compression="gzip")
    ...
    physical_counts = (
        ...
        .groupby(["fold", "horizon_steps", "condition"], observed=True)
        .size()
        .rename("condition_n")
        .reset_index()
    )
    group_cols = ["station", "fold", "horizon_steps", "method", "condition", "ramp_group"]
    for key, group in pred.groupby(group_cols, sort=True, observed=True):
        ...
        row["undercoverage_error"] = max(0.0, target - coverage)
        row["overcoverage_error"] = max(0.0, coverage - target)
    ...
    metrics_tmp = pd.DataFrame(conditional_rows)
    if not metrics_tmp.empty:
        metrics_tmp = metrics_tmp.merge(
            physical_counts,
            on=["fold", "horizon_steps", "condition"],
            how="left",
        )
        metrics_tmp["ramp_prevalence"] = np.where(
            metrics_tmp["ramp_group"].astype(str) == "ramp",
            metrics_tmp["n"] / metrics_tmp["condition_n"],
            np.nan,
        )
        conditional_rows = metrics_tmp.drop(columns=["condition_n"]).to_dict(orient="records")
    ...
    for key, group in ramp.groupby(
        ["station", "fold", "horizon_steps", "method", "condition", "ramp_direction"],
        observed=True,
        sort=True,
    ):
        if len(group) < 40:
    ...
    return conditional_rows, risk_rows, directional_rows
...
def candidate_gates(effects: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    rows: list[dict] = []
        ...
        rows.append(
            {
                ...
                "min_improvement": float(improvements.min()),
                "max_improvement": float(improvements.max()),
                "stations_improved_n": int((improvements > 0).sum()),
                "stations_improved_prop": float((improvements > 0).mean()),
                ...
                "max_method_worst_undercoverage": float(group["method_worst_undercoverage"].max()),
                "mean_clean_score_ratio": float(group["clean_score_ratio"].mean()),
                "comparative_gate_passed": bool(
                    improvements.mean() >= float(cfg["decision_rules"]["comparative_gate"]["improvement_over_baseline_at_each_horizon"])
                ...
                "absolute_reliability_gate_passed": bool(
                    group["method_worst_undercoverage"].max()
...
def decision_payload(
    ...
    if method_gate_methods:
        ...
        verdict = cfg["decision_rules"]["labels"]["short_paper"]
    else:
...
def main() -> None:
    ...
    for path in paths:
        ...
        c_rows, r_rows, d_rows = summarize_chunk(
            path,
    ...
    gates = candidate_gates(effects, cfg)
