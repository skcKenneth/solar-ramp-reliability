...
from solar_reliability.splits import leakage_safe_fold_masks  # noqa: E402
...
def fold_feasibility(
    ...
    for horizon in cfg["forecast"]["horizons_steps"]:
        ...
        for fold in cfg["data"]["site1_calendar_folds"]:
            ...
            ramp_n = int((meta.loc[masks.test, "ramp_fraction"] >= threshold).sum())
...
def confirmatory_support_exclusion_reason(feasible: int, checked: int) -> str | None:
    """Require complete support for the frozen four-fold, two-horizon matrix."""

    required = 8
    if feasible == checked == required:
        return None
    return (
        "incomplete frozen confirmatory support: "
        f"feasible={feasible}, checked={checked}, required={required}"
    )
...
def main() -> None:
    ...
    for row in manifest.itertuples(index=False):
        ...
        support_reason = confirmatory_support_exclusion_reason(feasible, checked)
